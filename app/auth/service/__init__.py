...
class AuthService:
    ...
    def register(self, data):
        ...
        if self.user_repo.find_by_username(data['username']):
            return None, None, f"The username '{data['username']}' is already taken."
        
        if self.user_repo.find_by_email(data['email']):
            return None, None, f"The email '{data['email']}' is already registered."
        ...
        try:
            ...
            return None, None, "An error occurred while saving the user."
    
    def login(self, email, password):
        if not email or not password:
            return None, None, "Both email and password are required."
        
        user = self.user_repo.find_by_email(email)
        
        if not user:
            return None, None, "No account found with this email address."
